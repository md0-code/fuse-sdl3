; ----------------------------------------------------------------------------------------  
; ZX Dandanator! Mini HW v1.0 Modified WR, HW2.0 - Eeprom Write SST39SF040 Unit to be included
;
; Typical Usage -> 	CALL DDNTRUNLOCK
;					LD A, SectN					; Loop 128 sectors
;					CALL SSTSECTERASE			;   |
;					LD A, SectN					;	|
...
;					JP DDNTRRESET
;
; Dandare - May 2017 - Verify included
; ----------------------------------------------------------------------------------------  	
...
;	 HL = RAM Address of sector to program : Source of data
;
; ***************  MUST BE RUN FROM RAM AND WITH EXTERNAL EEPROM PAGED IN  ***************
; ... Sector must be erased and programmed first
;  OUTPUT: Carry flag active for operations completed correctly. Carry flag inactive is operations fail
; ----------------------------------------------------------------------------------------
SSTVERIPROG:PUSH HL						; Save Ram Address
			PUSH AF						; Save Sector Number
			LD DE,0						; DE initialize to 0
			OR A						; Clear Carry flag
			RRA							; divide A by 2 and msbit to carry
			RR D						; catch carry to D (new carry is 0)
			RRA							; Convert N.Sector (0-127) to N.Slot (0-31).  divide A by 2 and msbit to carry
			RR D						; catch carry to D (new carry is 0)
			RR D						; move bits 7-6 to 6-5
			RR D						;Convert N.Sector (0-127) to High Byte of Slot Address for that sector. ; move bits 6-5 to 5-4
			; Now we have both, A=n.slot (0-31) and DE=Address into slot of 4k sector
			;PUSH DE					; SENDNRCMD  does not use DE, but if in future use it, uncomment this
			INC A						; Convert 0-31 to 1-32
			;LD A,xx					; Slot number 16k (1-32)
			LD HL,1
			CALL SENDNRCMD 				; tell PIC to select slot in A
	;ANULADO VERIFICAR SECTORES
	IF (1==0)
			;POP DE						; SENDNRCMD  does not use DE, but if in future use it, uncomment this
			POP AF
			POP HL
			PUSH HL
			PUSH AF
			LD BC,$1000					; Size or sector (4k)
BUCVERIFY:
			LD A,(DE)
			INC DE
			CPI
			JR Z,BUCVERIFY
			LD A,B
			OR C						;Check if BC=0, also clear carry flag
			JR NZ,ERRORVERIFY
	ENDIF
			POP AF
			POP HL
			SCF							; Operations were correct
			RET							; Return with Carry active
	IF (1==0)
ERRORVERIFY:
			POP AF
			POP HL
			RET							; Return with Carry inactive
	ENDIF
; ----------------------------------------------------------------------------------------	
...
; ----------------------------------------------------------------------------------------
; Unlock Dandanator Commands - MUST BE RUN AT THE BEGINNING OF THE CODE, Needs SP set
; - Also Pages in slot 2
; ----------------------------------------------------------------------------------------
DDNTRUNLOCK:LD E, 16					; Unlock Commands
			LD D, 16
			LD A, 46
			CALL SENDSPCMD
			CALL SENDCONF
...
; 
; ----------------------------------------------------------------------------------------
DDNTRRESET: LD A,1						; Command 1 -> Slot 0
			LD HL,1
			CALL SENDNRCMD
			CALL PAUSE
RESET:		RST 0
